...
import PostEditDeleteButton from '@/app/components/PostEditDeleteButton'
...
export default async function MyPosts() {
  const posts: IPost[] | null = await getMyPosts()
  ...
  return (
    <section className='flex flex-col items-center w-full h-auto'>
      <h1 className="mb-4 mt-4 text-2xl sm:text-2xl md:text-3xl lg:text-4xl font-rock text-white text-stroke-title text-shadow-xl">Your Posts</h1>
      <ul className='flex flex-col items-center h-auto max-h-[61vh] w-auto lg:min-w-[40vw] bg-[#A5A5A5] bg-opacity-[75%] border border-black rounded-md overflow-y-auto scrollbar-hide'>
        {posts ? posts.map((post) => (
           <li key={post._id} className='flex flex-col border-b border-black last-of-type:border-b-0 w-full p-2'>
           <div className='flex flex-col gap-1'>
             <Link href={`/forum/${post._id}/${post._id}`}><h3 className='text-white text-stroke-title text-lg md:text-xl lg:text-2xl xl:text-3xl font-bold'>{post.title}</h3></Link>
           ...
           <div className='flex flex-row justify-start items-center mt-2 gap-3'>
             <p className='text-white text-xxs md:text-xs lg:text-sm xl:text-base text-shadow-xl'>Posted: {formatDate(post.createdAt)}</p>
             <p className='text-white text-xxs md:text-xs lg:text-sm xl:text-base text-shadow-xl'>Replies: {post.repliesAmount}</p>
           </div>
           <div className='flex flex-row justify-between items-center mt-1.5 w-[35%]'>
             <PostEditDeleteButton postId={post._id}/>
           </div>
         </li>
        )) : posts === null ? (
