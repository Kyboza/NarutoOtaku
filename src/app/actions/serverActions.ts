  ...
  try {
    ...
    if (accessToken) {
      ...
      try {
        ...
        if (!visitingUser)
          throw new Error('No error with that id matching from Database')
      } catch (error) {
        handleError(error)
      }
    }
...
export async function getCharacter(characterId: string) {
  try {
    if (!characterId || !mongoose.Types.ObjectId.isValid(characterId))
      throw new Error('Did not recieve Character Id or its not a valid id')
...
export async function loadReplies(postId: string) {
  ...
  try {
    ...
    const filteredReplies = replies.filter(
      (reply) => reply.userId && reply.userId.username && reply.userId.imgPath
    )
