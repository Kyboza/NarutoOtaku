...
export async function POST(req: NextRequest){
    try {
        ...
        if(!characterName) return NextResponse.json({message: 'Did not recieve a character name to get likes and likers'}, {status: 500});

        const connection = await connectToDatabase()
        if(!connection.success) return NextResponse.json({message: connection.message}, {status: 500});

        const character = await Character.findOne({name: characterName}).select('likes likers');
        if(!character) return NextResponse.json({message: 'Could not find character with that name'}, {status: 404});
