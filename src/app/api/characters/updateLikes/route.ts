...
export async function POST(req: NextRequest){
    try{
        ...
        if(!ACCESS_SECRET) return NextResponse.json({message: 'Did not find accesstoken from env'}, {status: 500});

        const storedCookies = cookies()
        const accessToken = (await storedCookies).get('accessToken')?.value;
        if(!accessToken) return NextResponse.json({message: 'Cannot like character without accessToken'}, {status: 401});

        const decoded = jwt.verify(accessToken, ACCESS_SECRET);
        if(!decoded || typeof decoded !== 'object' || !('userId' in decoded)) return NextResponse.json({message: 'Possibly manipulated accessToken'}, {status: 403})
        ...
        const [user, character] = await Promise.all([
            ...
            Character.findOne({name: characterName}).select('likes userWhoLike name')
        ])

        if (!user || !character) {
            return NextResponse.json({ message: 'User or Character not found' }, { status: 404 });
        }

        if(visitingUser !== user?.username) return NextResponse.json({message: 'accessToken may have been manipulated'},  {status: 401});
