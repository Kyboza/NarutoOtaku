...
export async function DELETE(req: NextRequest) {
    try {
        ...
        if(!commentId) return NextResponse.json({message: 'No commentId provided'}, {status: 400});
        if(!mongoose.Types.ObjectId.isValid(commentId)) return NextResponse.json({message: 'commentId is not a valid ObjectId'}, {status: 400});
        ...
        const commentInComment = await Comment.findOneAndDelete({_id: commentId}, {userId: userIdFromToken});
        if(!commentInComment) return NextResponse.json({message: 'Could not find comment in comment collection'}, {status: 404});
        ...
        if(!commentInSpecific) return NextResponse.json({message: 'Could not remove comment from SpecificForum Collection'}, {status: 404});
