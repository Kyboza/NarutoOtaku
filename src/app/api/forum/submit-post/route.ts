...
export async function POST(req: NextRequest){
    ...
    if(!accessToken) return NextResponse.json({message: 'Missing accessToken to create post'}, {status: 403});

    const decoded = jwt.verify(accessToken, ACCESS_SECRET)
    if(!decoded || typeof decoded !== 'object' || !('userId' in decoded)) return NextResponse.json({message: 'Could not verify accessToken'}, {status: 401})
