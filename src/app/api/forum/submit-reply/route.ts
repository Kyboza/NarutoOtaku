...
export async function POST(req: NextRequest){
  try {
        ...
        if (!postId || !mongoose.Types.ObjectId.isValid(postId) || !replyContent) {
            return NextResponse.json({message: 'Missing information or id is invalid'}, {status: 400})
        }
    
        const storedCookies = cookies();
        const accessToken = (await storedCookies).get("accessToken")?.value;
        if (!accessToken) return NextResponse.json({message: 'Could not find accessToken'}, {status: 400})
            ...
            try {
                ...
                handleError(error)
                return NextResponse.json({ message: 'Invalid or expired accessToken' }, { status: 401 });
            }
    
            if (typeof decoded !== "object" || !decoded.userId || !('userId' in decoded)) {
                return NextResponse.json({ message: 'Invalid accessToken' }, { status: 401 });
            }
        ...
        if (!postItself) return NextResponse.json({message: 'Could not find post in database'}, {status: 400})
