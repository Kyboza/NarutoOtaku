...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!Array.isArray(items)) {
            console.log(items)
            return NextResponse.json({ message: "Invalid format: Items must be an array" }, { status: 400 });
        }
        ...
        try {
            ...
            return NextResponse.json({ id: newOrder._id.toString(), message: "Order created and pending" }, { status: 200 });
        } catch (error) {
            console.error("Database save error:", error);
            return NextResponse.json({ message: "Could not save order to database" }, { status: 500 });
        }
