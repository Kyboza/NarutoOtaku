  ...
  try {
    ...
    if (!refreshToken)
      return NextResponse.json(
        { message: 'No refresh token found user is not logged in' },
        { status: 401 }
      )
    ...
    if (
      ...
      return NextResponse.json(
        { message: 'Refresh token is invalid or it is missing content' },
        { status: 401 }
      )
    ...
    const response = NextResponse.json(
      { message: 'Successfully updated accessToken', accessToken },
      { status: 200 }
    )
    ...
    return NextResponse.json(
      { message: 'Failed to generate a new accessToken for user' },
      { status: 500 }
    )
  }
}
