...
export async function POST(req: NextRequest){
    ...
    if(!body) return NextResponse.json({message: "No register information recieved in body"}, {status: 400});
    ...
    if(!connection.success) return NextResponse.json({message: connection.message}, {status: 500})
        ...
        if(userExists){
            ...
            return NextResponse.json({message: "User with that username or email already exists"}, {status: 500})
        }
        ...
        await newUser.save()
        return NextResponse.json({message: 'Successfully posted user'}, {status: 200});
    }
    catch(error) {
       console.error("Could not register user", error);
       return NextResponse.json({message: "Could not register user"}, {status: 500})
    }
}