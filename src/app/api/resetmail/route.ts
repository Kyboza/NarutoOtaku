...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!EMAIL_USER || !EMAIL_PASS)
            throw new Error(
                "Could not get email user or pass variable from env file",
            )
        ...
        if (!email || !username)
            return NextResponse.json(
                {
                    message:
                        "Did not recieve any username or email from frontend",
                },
                { status: 400 },
            )
        ...
        if (!user)
            return NextResponse.json(
                {
                    message:
                        "No user found in database with that email or username",
                },
                { status: 400 },
            )
