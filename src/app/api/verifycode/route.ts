...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!email || !username || !code)
            return NextResponse.json(
                { message: "No code, email or username is missing" },
                { status: 400 },
            )
        ...
        if (!user)
            return NextResponse.json(
                { message: "Could not find a user with that code" },
                { status: 400 },
            )
        ...
        if (!comparison)
            return NextResponse.json(
                { message: "Code in db and provided code does not match" },
                { status: 400 },
            )
        ...
        return NextResponse.json(
            { message: "Codes matched successfully!" },
            { status: 200 },
        )
    } catch (error) {
