        ...
        <header className="relative top-0 z-10 flex h-[10vh] w-full items-center justify-evenly border-b border-black border-opacity-25 bg-[#D48900] drop-shadow-xl">
            ...
            >
                <div
                    className={`absolute top-[25%] h-[0.5vh] w-[3vh] rounded-md bg-white transition-all xl:w-[5vh] ${
                        isOpen
                            ? "animate-hamburgerOpen"
                            : "animate-hamburgerClose"
                    }`}
                ></div>
                <div
            ...
            <div className="flex w-[25vw] items-center justify-center gap-4">
                ...
                <Link href={active ? `/users/${userName}` : "/login"}>
                    <FaUserAlt className="mr-2 text-2xl text-white drop-shadow-xl md:text-3xl lg:text-4xl" />
                </Link>
            </div>

            <ul
                ref={listRef}
                className={`min-h[25vh] absolute left-0 top-[100%] flex h-auto w-screen flex-col items-center justify-evenly rounded-md rounded-tl-none rounded-tr-none border border-black bg-[#A5A5A5] bg-opacity-90 transition-all duration-300 ease-in-out last-of-type:border-b-0 ${isOpen ? "max-h-[500px] opacity-100" : "max-h-0 overflow-hidden opacity-0"}`}
            >
