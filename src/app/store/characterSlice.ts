...
export const fetchLikeInfo = createAsyncThunk(
    "characters/fetchLikeInfo",
    async (characterName: string, { rejectWithValue }) => {
        try {
            const response = await axiosAPI.post("/api/characters/getLikes", {
                characterName,
            })
...
export const updateLikeInfo = createAsyncThunk(
    "characters/updateLikeInfo",
    async (
        data: { visitingUser: string; characterName: string },
        { rejectWithValue },
    ) => {
        try {
            const response = await axiosAPI.post(
                "/api/characters/updateLikes",
                {
                    data,
                },
            )
    ...
    reducers: {
        handleLike: (state, action: PayloadAction<string>) => {
            ...
            if (state.userWhoLike.includes(visitingUser)) {
                state.userWhoLike = state.userWhoLike.filter(
                    (user) => user !== visitingUser,
                )
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchLikeInfo.fulfilled, (state, action) => {
                ...
                state.userWhoLike = Array.isArray(
                    action.payload.data.userWhoLike,
                )
                    ? action.payload.data.userWhoLike
                    : []
                state.error = null
            ...
            .addCase(fetchLikeInfo.rejected, (state, action) => {
                state.error =
                    ...
                    "Unknown Error Fetching Likes and Likers"
                state.loading = false
            })

        builder
            .addCase(updateLikeInfo.fulfilled, (state, action) => {
                state.likes = action.payload.data.likes
            ...
            .addCase(updateLikeInfo.rejected, (state, action) => {
                state.error =
                    action.error?.message ||
                    "Unknown error while updating likes for character"
                state.loading = false
            })
    },
})
